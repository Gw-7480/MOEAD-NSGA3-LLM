...
# 用于从当前种群中选择个体以生成下一代。
    ...
    def crossoverOperator(self, population):
        ...
        for i in range(0, self.popSize, 2):  # 每次处理两个个体
            ...
            # # 卸载因子交叉 试试如果将卸载因子先不急交叉会怎样
            # for key in ind_k.offloading_factors:
            #     if np.random.rand() < 0.5:
            #         # 均匀交叉
            #         ind_k.offloading_factors[key], ind_l.offloading_factors[key] = (
            #             ind_l.offloading_factors[key], ind_k.offloading_factors[key]
            #         )
    ...
    def mutantOperator(self, population):
        """for ind in population:  # 遍历种群中的每个个体
            ...
            # # 卸载因子变异 暂时先试试将卸载因子恒定
            # for key in ind.offloading_factors:
            #     if np.random.rand() < 0.1:  # 变异概率可调
            #         ind.offloading_factors[key] = np.clip(
            #             ind.offloading_factors[key] + np.random.normal(0, 0.1), 0, 1
            #         )
    ...
    def calculate_satellite_offloading_metrics(
        ...
        data_transmission_time = (b * (1 - mu_i_l) * (1 - mu_i_n) * (
                    1 - mu_i_m) * W_i_bit) / R_ij  # bit / (bit/s) = s
...
class MyProblem(Problem):
    def __init__(self, n_var, n_obj, xl, xu, instance_name, args):
        super().__init__(n_var=n_var, n_obj=n_obj, xl=xl, xu=xu)
        self.instance_name = instance_name
        self.args = args
        self.fitness_evaluator = NSGA2(instance_name, args)
        # 加载DAG依赖关系
    ...
    def _load_dag_dependencies(self):
        ...
        dag_info = {}
        try:
            # 修正路径计算 - fitness.py已经在MODTSRA目录下
            current_file_dir = os.path.dirname(os.path.abspath(__file__))
            # 从 MODTSRA/ 直接访问 instance/ 子目录
            instance_path = os.path.join(current_file_dir, 'instance', self.instance_name, 'DAG')

            print(f"MyProblem: Looking for DAG files in: {instance_path}")
        ...
        while queue:
            ...
            if current in dependencies:
                for successor in dependencies[current]['successors']:
                    if successor < self.args.Nij and successor in in_degree:
                        in_degree[successor] -= 1
                        if in_degree[successor] == 0:
                            queue.append(successor)

        # 如果结果长度不等于任务数，说明有环或其他问题
        if len(result) != self.args.Nij:
            # 返回简单的顺序排列作为备选
            return list(range(self.args.Nij))

        return result

    def _validate_and_fix_order(self, order_sequence, uav_index):
        """验证并修正任务执行顺序"""
        try:
            # 检查是否包含所有必要的任务ID
            unique_tasks = set(order_sequence)
            expected_tasks = set(range(self.args.Nij))

            # 如果任务ID不完整或有重复，重新生成
            if unique_tasks != expected_tasks:
                print(f"Warning: Invalid task sequence for UAV {uav_index}, regenerating...")
                return self._generate_valid_topological_order(uav_index)

            # 验证拓扑排序约束
            if uav_index in self.dag_dependencies:
                dependencies = self.dag_dependencies[uav_index]
                task_positions = {task: pos for pos, task in enumerate(order_sequence)}

                # 检查所有依赖关系是否满足
                for task_id, info in dependencies.items():
                    if task_id in task_positions:
                        for pred in info['predecessors']:
                            if pred in task_positions:
                                if task_positions[pred] >= task_positions[task_id]:
                                    # 违反了拓扑排序，重新生成
                                    print(
                                        f"Warning: Topological constraint violated for UAV {uav_index}, regenerating...")
                                    return self._generate_valid_topological_order(uav_index)

            return list(order_sequence)

        except Exception as e:
            print(f"Error validating order for UAV {uav_index}: {e}, using fallback")
            return self._generate_valid_topological_order(uav_index)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        results = []
        for i in range(N):
            xi = x[i] if N > 1 else x
            xi = np.asarray(xi).flatten()  # 保证xi是一维

            # 变量分割
            order_flat = xi[:len_order].astype(int)
            location_flat = xi[len_order:len_order + len_location].astype(int)
            A = xi[len_order + len_location:len_order + len_location + len_A]

            # 确保A的和为1
            if np.sum(A) > 0:
                A = A / np.sum(A)
            else:
                A = np.ones(len_A) / len_A

            # B部分如原来处理
            from MODTSRA.Utils import N_service
            B = np.random.randint(0, 2, size=N_service)

            uav_list = []
            for j in range(N_n):
                uav = UAV(K)
                uav.workflow = copy.deepcopy(self.fitness_evaluator.workflow_set[j])

                # 提取该UAV的order序列并验证修正
                start_idx = j * Nij
                end_idx = (j + 1) * Nij
                raw_order = order_flat[start_idx:end_idx]

                # 验证并修正任务执行顺序
                valid_order = self._validate_and_fix_order(raw_order, j)
                uav.workflow.order = valid_order

                # location需要确保在有效范围内
                raw_location = location_flat[start_idx:end_idx]
                valid_location = np.clip(raw_location, 1, K + 1).tolist()
                uav.workflow.location = valid_location

                # ratio
                uav.ratio = A[j]
                # 传输速率
                uav.R_i = self.fitness_evaluator.UAV_transmission_rate_set[j]
                uav_list.append(uav)

            ind = Individual()
            ind.chromosome['S'] = uav_list
            ind.chromosome['A'] = A
            ind.chromosome['B'] = B

            print(f"[DEBUG] Individual {i}:")
            print(f"  order_flat (raw): {order_flat}")
            print(f"  location_flat: {location_flat}")
            print(f"  A: {A}")
            for j, uav in enumerate(uav_list):
                print(f"  UAV {j} valid order: {uav.workflow.order}")
                print(f"  UAV {j} valid location: {uav.workflow.location}")

            try:
                self.fitness_evaluator.calculateFitness(ind)
                results.append(ind.fitness[:3])
            except Exception as e:
                print(f"Error calculating fitness for individual {i}: {e}")
                # 返回惩罚值
                results.append([1e6, 1e6, 1e6])

        out["F"] = np.array(results) if N > 1 else np.array(results[0])
