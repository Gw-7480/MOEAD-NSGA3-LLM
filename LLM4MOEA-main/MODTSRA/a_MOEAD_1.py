...
class MOEAD_1:
    ...
    def run(self):
        ...
        t = 1
        # 新增：保存每一代PF的路径
        pf_gen_dir = os.path.join(project_path, 'ExperimentResult', self.instance_name, 'MOEAD_1', '1', 'PF_gen')
        ...
        while (t <= self.maxGen):
            ...
            # 新增：保存每一代EP的归一化适应度
            pf_gen = [copy.deepcopy(ind.fitness) for ind in self.EP]
    ...
    def reproduction(self, i):
        ...
        for gene in self.population[k].chromosome['S']:
            uav_k = copy.deepcopy(gene)
            self.reInitialize_WorkflowTaskSet_Schedule(uav_k)
        ...
        for gene in self.population[l].chromosome['S']:
            uav_l = copy.deepcopy(gene)
            self.reInitialize_WorkflowTaskSet_Schedule(uav_l)
    ...
    # 现在R_nl容易偏小 需要后期再重新看 因为B_n传参一直是0.01 所以就会导致R_nl偏小，再运行两遍看看是否一直正确 现在R_nl大概是六万比特或者十几万
    ...
    def calculateFitness(self, ind):
        ...
        for index in range(N_n):
            ...
            # 再计算地面到无人机的延时
            # ground_delay = self.calculate_ground_execution_delay(uav, uav.workflow)
            # time.append(ground_delay)
            ...
            # task_energy = 0
            # # 1. 地面本地计算能耗
            # task_energy += self.calculateLocalEnergy(self.instance_name, 1)
            # energy.append(task_energy)
            # energy.append(uav.workflow.schedule.E_total)
            # 使用新的能耗计算方法
            # individual_energy = self.calculate_individual_energy(ind)
            # energy.append(individual_energy)
...
def MOEAD_1_run(instance_name, args):
    alg_dir = project_path + '/ExperimentResult/' + instance_name + '/MOEAD_1/'
    ...
    for I in range(1, args.runTime + 1):
        ...
        # 新增：设置参考帕累托前沿
        from MODTSRA.IGD import get_combined_pf_ref
...
if __name__ == '__main__':
    ...
    args.save_dir = os.path.join(args.save_dir, instance_name, 'MOEAD_1')
