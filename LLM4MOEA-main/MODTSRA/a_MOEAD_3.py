'''''
[1] Song F , Xing H , Luo S ,et al.A Multiobjective Computation Offloading Algorithm 
for Mobile Edge Computing[J].
IEEE Internet of Things Journal, 2020, PP(99):1-1.DOI:10.1109/JIOT.2020.2996762.
'''''
import copy
...
class MOEAD_3:
    ...
    def reproduction(self, i):
        ...
        if r <= 0.8:
            ...
            # 邻居个体与非支配解集中的任意选择的个体进行交叉和变异
            nei_index = random.choice(self.B[i])
            self.crowding_distance_assignment(self.EP)
            self.EP = sorted(self.EP, key=lambda Individual: Individual.distance, reverse=True)  # sort population by Individual.temp_fitness
            index = random.randint(0, int(len(self.EP)*0.2))
            ind_non_temp = self.EP[index]
            ind_nei = Individual()
            ind_non = Individual()

            uav_set = []
            for gene in self.population[nei_index].chromosome['S']:
                uav_i = copy.deepcopy(gene)
                self.reInitialize_WorkflowTaskSet_Schedule(uav_i)
                uav_set.append(uav_i)
            ind_nei.chromosome = {'S': uav_set,
                                'A': copy.copy(self.population[nei_index].chromosome['A']),
                                'B': copy.copy(self.population[nei_index].chromosome['B'])}

            uav_non_set = []
            for gene in ind_non_temp.chromosome['S']:
                uav_non = copy.deepcopy(gene)
                self.reInitialize_WorkflowTaskSet_Schedule(uav_non)
                uav_non_set.append(uav_non)
            ind_non.chromosome = {'S': uav_non_set,
                                'A': copy.copy(ind_non_temp.chromosome['A']),
                                'B': copy.copy(ind_non_temp.chromosome['B'])}
            self.crossoverOperator(ind_nei, ind_non)
            self.mutantOperator(ind_nei)
            self.mutantOperator(ind_non)
            self.calculateFitness(ind_nei)
            self.calculateFitness(ind_non)
            return self.select_best_individual(ind_nei, ind_non)
    ...
    def fast_non_dominated_sort_temp(self, population):
        for p in population:
            p.S_p = []
            p.rank = None
            p.n = 0
...
def MOEAD_3_run(instance_name, args):
    alg_dir = project_path+'/ExperimentResult/' + instance_name + '/MOEAD_3/'
    ...
    for I in range(1, args.runTime + 1):
        ...
        startTime = time.time()
        moead = MOEAD_3(instance_name, args)
        EP_list = moead.run()
        endTime = time.time()
...
if __name__ == '__main__':
    ...
    args.save_dir = os.path.join(args.save_dir, instance_name, 'MOEAD_3')
    ...
    MOEAD_3_run(instance_name, args)
