    ...
    def make_new_population(self, population):  # 遗传操作产生新种群
        ...
        for ind in selectionResult:
            ...
            for gene in ind.chromosome['S']:
                uav = copy.deepcopy(gene)
                self.reInitialize_WorkflowTaskSet_Schedule(uav)
                uav_set.append(uav)
            new_ind.chromosome = {'S': uav_set,
                                  'A': copy.copy(ind.chromosome['A']),
                                  'B': copy.copy(ind.chromosome['B'])}
    ...
    def mutantOperator(self, population):
        for ind in population:
            ...
            for gene in ind.chromosome['S']:
                ...
                if (rnd_UAV < 1.0 / N_uav):  # 针对每一个基因（UAV）判断是否变异
