...
def MOEAD_DVFS_run(instance_name, args):
    alg_dir = project_path+'/ExperimentResult/' + instance_name + '/MOEAD_DVFS/'
    ...
    for I in range(1, args.runTime + 1):
        ...
        startTime = time.time()
        moead = MOEAD_DVFS(instance_name, args)
        EP_list = moead.run()
        endTime = time.time()
...
if __name__ == '__main__':
    ...
    print_info('************************  MOEA/D-DVFS ************************')
    print_info(
        '\n---------------------- Test instance:  [' + str(args.Nij) + ',' + str(args.K) + ']------------------------')

    '''创建测试集目录'''
    instance_name = '[' + str(args.Nij) + ',' + str(args.K) + ']'
    args.save_dir = os.path.join(args.save_dir, instance_name, 'MOEAD_DVFS')
    ...
    MOEAD_DVFS_run(instance_name, args)
