'''''
Ma S , Song S , Yang L ,et al.Dependent tasks offloading based on 
particle swarm optimization algorithm in multi-access edge computing[J].
Applied Soft Computing, 2021(112-):112.
'''''
import copy
...
class MOPSO:
    ...
    def run(self):
        ...
        self.update_global_best_particle(self.EP)

        t = 1
        while (t <= self.maxGen):
            inertial_weight = 0.5 + (0.9 - 0.5) * (t/self.maxGen)
            for ind in self.population:
                self.update_particle_location_velocity(inertial_weight, ind)
                for uav in ind.chromosome['S']:
                    self.reInitialize_WorkflowTaskSet_Schedule(uav)
                self.calculateFitness(ind)
                self.DVFS_Schedule_Algorithm(ind)
                self.update_history_best_partical(ind)
                self.update_EP_FromElement(self.EP, ind)
                self.update_global_best_particle(self.EP)
    ...
    def DVFS_Schedule_Algorithm(self, ind):
        ...
        for gene in ind.chromosome['S']:
            ...
            for coreId in schedule.S:
                if coreId < self.K and schedule.S[coreId] != []:
                    for i in range(len(schedule.S[coreId])):
                        ...
                        while flag == 0 and m < self.M:
                            ...
                            if i != (len(schedule.S[coreId]) - 1):  # There is next task v_j on the same core
                                ...
                                limit1 = workflow.schedule.T_total
                            if vi.id != workflow.exitTask:
                                limit2 = self.get_min_Succ_ST(workflow, vi)
                            else:
                                limit2 = workflow.schedule.T_total
                            if FT_i_new <= limit1 and FT_i_new <= limit2:
                                flag = 1
                                vi.actualFre = m
                                vi.FT_i_l = FT_i_new
            ...
            energy.append(schedule.E_total)
        ind.fitness[1] =  np.average(energy) # 只改变能耗
    ...
    def initializePopulation(self):
        for i in range(self.popSize):
            ...
            ind.velocity = {'A': [random.uniform(-6, 6) for _ in range(N_uav)], 'B': [random.uniform(-6, 6) for _ in range(N_service)]}
            self.calculateFitness(ind)
            self.population.append(ind)
            ind.hb_particle.chromosome = copy.deepcopy(ind.chromosome)
            ind.hb_particle.fitness = copy.deepcopy(ind.fitness)


    def update_global_best_particle(self, population):
        self.crowding_distance_assignment(population)
        population = sorted(population, key=lambda Individual: Individual.distance, reverse=True)
        N_temp = int(0.2 * len(population)) # 从前20%的个体中随机选择一个非支配解作为全局最优解
        i = random.randint(0, N_temp)
        self.gb_particle = Pareto()
        self.gb_particle.chromosome = copy.deepcopy(population[i].chromosome)
        self.gb_particle.fitness = copy.deepcopy(population[i].fitness)
    ...
    def crowding_distance_assignment(self, population):  # 计算拥挤距离
        ...
        for m in range(obj_num):
            ...
            population = sorted(population, key=lambda Individual: Individual.temp_fitness)  # sort population by Individual.temp_fitness
            ...
            if (population.__len__() == 1):
                ...
                for i in range(1, population.__len__() - 1):
                    population[i].distance += (population[i+1].fitness[m] - population[i-1].fitness[m]) / (f_max-f_min+1)  # 加一为了防止分母为零
    ...
    def update_particle_location_velocity(self, inertial_weight, ind):
        # 针对分配比率
        for j in range(N_uav):
            r1 = random.random()
            r2 = random.random()
            v_j = inertial_weight * ind.velocity['A'][j] + \
                  2 * r1 * (ind.hb_particle.chromosome['A'][j] - ind.chromosome['A'][j]) + \
                  2 * r2 * (self.gb_particle.chromosome['A'][j] - ind.chromosome['A'][j])
            if v_j <= -6:
                v_j = -6
            elif v_j >= 6:
                v_j = 6
            ind.velocity['A'][j] = v_j
            ind.chromosome['A'][j] = ind.chromosome['A'][j] + v_j
        ...
        for j in range(N_service):
            r1 = random.random()
            r2 = random.random()
            v_j = inertial_weight * ind.velocity['B'][j] + \
                  2 * r1 * (ind.hb_particle.chromosome['B'][j] - ind.chromosome['B'][j]) + \
                  2 * r2 * (self.gb_particle.chromosome['B'][j] - ind.chromosome['B'][j])
            if v_j <= -6:
                v_j = -6
            elif v_j >= 6:
                v_j = 6
            ind.velocity['B'][j] = v_j
            sigmoid_y = 1. / (1 + np.exp(v_j))
            if random.random() < sigmoid_y:
                ind.chromosome['B'][j] = 1
            else:
                ind.chromosome['B'][j] = 0
    ...
    def update_history_best_partical(self, ind):
        if self.isDominated(ind.fitness, ind.hb_particle.fitness):
            ind.hb_particle.chromosome = copy.deepcopy(ind.chromosome)
            ind.hb_particle.fitness = copy.deepcopy(ind.fitness)
        elif self.isDominated(ind.hb_particle.fitness, ind.fitness):
            pass
        else:
...
def MOPSO_run(instance_name, args):
    alg_dir = project_path+'/ExperimentResult/' + instance_name + '/MOPSO/'
    ...
    for I in range(1, args.runTime + 1):
        ...
        startTime = time.time()
        naga2 = MOPSO(instance_name, args)
        EP_list = naga2.run()
...
if __name__ == '__main__':
    ...
    args.save_dir = os.path.join(args.save_dir, instance_name, 'MOPSO')
