...
class Algorithm:
    ...
    def infill(self):
        ...
        # 如果算法还没有初始化（第一次调用 next），则进行初始化
        if not self.is_initialized:

            # 预初始化钩子，供子类扩展（如提前分配资源等）
            self._initialize()

            # 执行初始化阶段的 infill（如生成初始种群等
            infills = self._initialize_infill()

        else:
            # 如果已经初始化，调用常规 infill 方法生成新解（如进化产生子代）
            infills = self._infill()

        # 如果 infill 阶段生成了解（如新种群/子代），则记录当前迭代次数
        if infills is not None:
            infills.set("n_gen", self.n_iter)  # 设置当前代数
            infills.set("n_iter", self.n_iter)  # 设置当前迭代次数（有些算法区分代数和迭代）
