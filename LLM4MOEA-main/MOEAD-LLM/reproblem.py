...
"""
import numpy as np
from MODTSRA.fitness import NSGA2, Individual
from MODTSRA import Utils
from MODTSRA.fitness import UAV
from MODTSRA.fitness import UAV, Individual
import copy
import random  # 确保已导入
...
class RE21:
    def __init__(self):
        self.problem_name = 'RE21'  # 问题名称
        self.n_objectives = 2       # 目标函数个数
        self.n_variables = 4        # 决策变量个数
        self.n_constraints = 0      # 约束条件个数（归一化后为0）
        self.n_original_constraints = 0  # 原始约束条件个数（为0）

        F = 10.0                    # 外部载荷（常数）
        sigma = 10.0                # 允许应力（常数）
        tmp_val = F / sigma         # 计算变量下界的中间变量

        self.ubound = np.full(self.n_variables, 3 * tmp_val)  # 所有变量的上界为 3*tmp_val
        self.lbound = np.zeros(self.n_variables)              # 初始化所有变量下界为0
        self.lbound[0] = tmp_val                              # 第1个变量下界为 tmp_val
        self.lbound[1] = np.sqrt(2.0) * tmp_val               # 第2个变量下界为 sqrt(2)*tmp_val
        self.lbound[2] = np.sqrt(2.0) * tmp_val               # 第3个变量下界为 sqrt(2)*tmp_val
        self.lbound[3] = tmp_val                              # 第4个变量下界为 tmp_val

    def evaluate(self, x):
        f = np.zeros(self.n_objectives)   # 初始化目标函数值数组
        x1 = x[0]                         # 第1个决策变量
        x2 = x[1]                         # 第2个决策变量
        x3 = x[2]                         # 第3个决策变量
        x4 = x[3]                         # 第4个决策变量
        ...
        sqrt_2 = np.sqrt(2.0)             # 计算 sqrt(2)

        # 目标1：结构重量或体积相关目标
        f[0] = L * ((2 * x1) + sqrt_2 * x2 + np.sqrt(x3) + x4)
        # 目标2：结构变形或应力相关目标
        f[1] = ((F * L) / E) * ((2.0 / x1) + (2.0 * sqrt_2 / x2) - (2.0 * sqrt_2 / x3) + (2.0 / x4))
