...
class RE(ElementwiseProblem):
    def __init__(self, idx):
        # 1. 选择RE系列的具体问题（如RE21、RE22等），mop_rw是这些问题类的元组
        self.p = mop_rw[idx]()
        # 2. 构造理想点和纳迪尔点的数据文件路径
        ...
        self.zu = np.ones(self.M, float) + .1  # 超体积计算时的上界
        # 5. 初始化父类ElementwiseProblem，设置变量数、目标数、变量上下界等
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        # 归一化目标函数值：(f(x) - ideal) / (nadir - ideal)
